...
# Load FX rates for currency conversion of global firms
df_fx <- read_csv(
...
# (some firms appear in both tables; keep first occurrence)
firm_all <- firm_us %>%
  select(gvkey, fyear, conm, at, revt, emp,
         roa, ros, tobin_q, naics, sic) %>%
  ...
  filter(dup_id == 1) %>%
  select(-dup_id) %>%
  mutate(naics2 = str_sub(as.character(naics), 1, 2),
...
#    Join key: gvkey + year
#    The colocation data uses `year` (the annual report year).
#    The financial data uses `fyear` (fiscal year).
#    We match them directly; firms whose report year differs from fiscal year
#    will have a small mismatch — acceptable for this analysis.
# =============================================================================
df_merged <- firm_all %>%
  inner_join(
    df_colocation %>%
...
# Quick check: how many firms have valid Tobin's Q?
...
#
#   Notation used below:
#     comp  = n_complementary_keyword   (main independent variable)
#     ind   = n_independent_keyword     (control)
#     press = n_pressure_keyword        (control)
#     at    = total assets              (size control)
#     emp   = employees                 (size control)
#
#   Fixed effects (FEs) are stacked using fixest's csw0() and csw():
#     csw0(year, naics2) means: run three models with FEs
#       (1) no FE  (2) year FE only  (3) year + naics2 FE
#     csw(year, naics2) is the same but always includes at least one FE
#
...
#   C: Mining subsample only, NAICS2 industry FE
#   D: Mining subsample only, SIC2 industry FE
# =============================================================================
...
save_reg_table(reg_roa_A, "tab_reg_roa_fullsample_naics2")
save_reg_table(reg_roa_B, "tab_reg_roa_fullsample_sic2")
save_reg_table(reg_roa_C, "tab_reg_roa_mining_naics2")
save_reg_table(reg_roa_D, "tab_reg_roa_mining_sic2")
...
save_reg_table(reg_ros_A, "tab_reg_ros_fullsample_naics2")
save_reg_table(reg_ros_B, "tab_reg_ros_fullsample_sic2")
save_reg_table(reg_ros_C, "tab_reg_ros_mining_naics2")
save_reg_table(reg_ros_D, "tab_reg_ros_mining_sic2")
...
reg_tobinq_A <- feols(
  ...
  + at + emp | csw0(fyear, naics2),
  vcov = "HC1",
  data = df_merged %>% filter(!is.na(tobin_q))   # drop rows with no valid Q
)
...
save_reg_table(reg_tobinq_A, "tab_reg_tobinq_fullsample_naics2")
save_reg_table(reg_tobinq_B, "tab_reg_tobinq_fullsample_sic2")
save_reg_table(reg_tobinq_C, "tab_reg_tobinq_mining_naics2")
save_reg_table(reg_tobinq_D, "tab_reg_tobinq_mining_sic2")


# =============================================================================
# 7. Combined display tables (one table per dependent variable,
#    showing full sample vs mining subsample side by side)
# =============================================================================
...
save_reg_table(list(reg_roa_A,    reg_roa_C),    "tab_reg_roa_combined")
save_reg_table(list(reg_ros_A,    reg_ros_C),    "tab_reg_ros_combined")
save_reg_table(list(reg_tobinq_A, reg_tobinq_C), "tab_reg_tobinq_combined")
